...
namespace MultiTerminal.Terminal
{
    ...
    public class WebViewTerminalRenderer : UserControl
    {
        ...
        ///
        /// <para>⚠️ WHAT IT DOES NOT DO (task 8b270b37). This comment used to say it "detects if
        /// Claude is busy by monitoring terminal output changes" and "retries with exponential
        /// backoff". Neither was ever true: the output time was copied into a local nothing read, and
        /// no delay is taken between attempts. A retry happens ONLY when a send fails — an attempt
        ...
        /// <returns>True when one attempt's send was acknowledged (Method 1) or completed without throwing (Methods 2 and 3) — the keypress was delivered, NOT that a prompt was submitted.</returns>
        private async System.Threading.Tasks.Task<bool> SendEnterWithRetryAsync(int maxRetries, int initialDelayMs, System.Threading.CancellationToken cancellationToken)
        {
            ...
            for (int attempt = 0; attempt < maxRetries; attempt++)
            {
                ...
                // The send was acknowledged (Method 1) or completed without throwing (Methods 2 and 3).
                // ⚠️ NOT "the Enter was processed", which this used to claim (task 8b270b37): the
                // page acks on ANY cursor movement — including a CR the composer took as a newline —
                // and acks again when its own 500ms poll expires.
