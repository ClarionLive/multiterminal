...
#pragma warning restore CA2213
        ...
        private void WireTasksPanelEvents(TasksPanelDocument panel)
        {
            ...
            // Board HUD layout persistence (task f5744489). Wired HERE, with the board's other
            // events, for the reason in this method's summary: the board has three construction
            // sites, and anything wired at only one of them is dead at the other two with no error.
            //
            // No cross-panel propagation loop, unlike the TerminalDocument zoom fan-out: those three
            // sites are alternative ways to create THE board, not three simultaneous boards, so
            // there is no sibling to propagate to. Writing to settings is the whole job.
            panel.HudTabZoomChanged += (s, e) => _settings?.SetHudTabZoom(e.ZoomKey, e.Zoom);
            panel.HudSplitRatioChanged += (s, ratio) => _settings?.SetTasksPanelHudSplitRatio(ratio);
        }

        // OnPlanGraphRequested was DELETED here (task f5744489), and it is worth recording what it
        // did rather than leaving a silent gap. It turned "which card was clicked" into "whose HUD
        // do I commandeer": the assignee's terminal if one could be matched by name, otherwise
        // merely _lastActiveTerminal — so an UNASSIGNED card landed in whichever terminal the Owner
        // happened to touch last, a terminal chosen by accident and now displaying a ticket it had
        // no relationship to.
        //
        // No host-side resolution replaces it, because the question no longer arises. The board's
        // own HUD lives in the board's panel and shows the selected card; nothing has to decide
        // whose window to borrow. See TasksPanelDocument.BuildBoardHud.
