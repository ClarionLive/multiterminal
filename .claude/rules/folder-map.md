...
| `MCPServer/Services/` | MCP server services | **MessageBroker**, PoolCoordinator, ActivityService, StaleTaskService, SpawnService, HttpWebhookService, SessionDiscovery, ComplexityDetector, SummaryService, GlossBackfillService + GlossBackfillPlanner (auto-writes missing 🔗 Plan-tab glosses, task a455e295), AgentAttentionService (which sessions are blocked ON THE OWNER and for how long — set from `NotificationReceived`, cleared by observed main-thread activity; task 2289bb8a), AgentActivityWatcher (task edcdcdd5 — polls `activity_feed` by row id and feeds AgentAttentionService: this is the CALLER that clear path went without, which is why blocks used to set and never clear. `TOOL_COMPLETE`/`TOOL_FAILED` clear; `TURN_END` → Idle is the edge for a prompt the owner dismissed with Escape; `TOOL_START` feeds the display line ONLY and must never clear, because it is written on PreToolUse and `safety-hook.js` is itself a PreToolUse hook returning `ask`. A row that cannot say whether it came from a subagent counts as one — a false clear renders a CALM CARD, indistinguishable from nobody needing you) |
