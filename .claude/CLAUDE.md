...
- **⚠️ READ THE STAMP — NEVER TRUST A CLAIM ABOUT WHAT IS IN `staged` OR `Deploy`.** Both folders carry a `.build-info.json` recording the exact branch + commit + source worktree that last wrote them. **That file is the only authority.** A ticket card, a continuation note, a sibling agent, or this document telling you what is deployed is a *claim*, and claims here go stale constantly — any worktree's build silently re-clobbers `staged`, so it frequently holds a task branch rather than `main`. Before you conclude a change is or isn't live, before any deploy, and before believing a card's "not yet live" blocker: `cat` the stamp. Recorded incidents: three cards simultaneously asserting a `staged` commit that was wrong (a deploy would have *removed* a working fix that was live but not on `main`); and a card asserting `Deploy` was `fff6488` when the stamp read `5387dc0`, which had already made the ticket's standing "never executed live" blocker false.
