...
error returns), plus the deliberate `{success}`-bearing result DTOs. They are out of scope for
the error-shape contract by construction (the contract governs error-STATUS responses only):
...
- **`MessagingController` POST /messaging/send + POST /messaging/broadcast**, and
  **`TasksController` GET /tasks/inbox/{userId}**, return typed result-object DTOs
  (`SendResult`, `BroadcastResult`, `GetInboxResult`) that carry a `success` bool and an
  `error` string as first-class 200-body fields. These model **expected partial failures as
  data** (a target may be offline; a broadcast reports per-recipient delivery / a count) rather
  than throwing ProblemDetails for a normal, non-exceptional outcome — a deliberate result-object
  design, arguably better than an error-STATUS for expected conditions, not envelope debt. Do
  not unwrap or convert (PM-ratified, 68a615a4).
